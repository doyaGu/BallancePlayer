...
bool EditScript(CKLevel *level, const BpGameConfig &config) {
    ...
    // Set debug mode
    if (config[BP_CONFIG_DEBUG] == true) {
    ...
    if (!SetLanguage(scriptutils::GetBehavior(defaultLevel, "Set Language"), config[BP_CONFIG_LANG_ID].GetInt32()))
    ...
    if (!bbpFilter) {
        ...
        scriptutils::SetInputParameterValue(bbpFilter, 2, config[BP_CONFIG_BPP].GetInt32());
    }

    // Unlock widescreen (Not 4:3)
    if (config[BP_CONFIG_UNLOCK_WIDESCREEN] == true) {
        if (!UnlockWidescreen(sm, minWidth))
            bpLogWarn("Failed to unlock widescreen");
    }

    // Unlock high resolution
    if (config[BP_CONFIG_UNLOCK_HIGH_RESOLUTION] == true) {
        if (!UnlockHighResolution(sm, bbpFilter, minWidth, maxWidth, config[BP_CONFIG_UNLOCK_WIDESCREEN] == true))
    ...
    if (config[BP_CONFIG_UNLOCK_FRAMERATE] == true) {
    ...
    if (config[BP_CONFIG_SKIP_OPENING] == true) {
